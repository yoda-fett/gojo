...
import { logLaundryOut } from '@/lib/services/housekeeping-laundry';
  ...
  items: z
    .array(
      z.object({
        ...
        dirtyPulled: z.number().int().min(0).max(1000),
        cleanPlaced: z.number().int().min(0).max(1000),
      }),
    )
    .min(1),
...
export const POST = withAuth(async (req, actor) => {
  ...
  try {
    ...
    const result = await logLaundryOut(actor, { ...body, idempotencyKey });
